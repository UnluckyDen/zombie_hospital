...
namespace Trepanation.UnityComponents
{
    public class EmitterUc : MonoBehaviour
    {
        ...
        public void SetCursorPosition(Vector2 position, AnimationType animationType)
        {
            _show = true;
            ...
            if (animationType == AnimationType.Click) _cursorAnimator.Play("Click");
            else StartCoroutine(nameof(PlayAfter));
        }
        ...
        private IEnumerator Swipe()
        {
            ...
            while (Mathf.Abs(cursor.anchoredPosition.x - _endPoint.x) > 0.1f && 
                   Mathf.Abs(cursor.anchoredPosition.y - _endPoint.y) > 0.1f)
            {
